...
const INITIAL_ACCOUNTS = [
  { id:1, username:'ali_cars_pk',    site:'OLX Pakistan', status:'active',  lastSync:'2 mins ago',   adsCount:14, color:'#10b981' },
  { id:2, username:'zameen_listings', site:'Zameen.com',   status:'active',  lastSync:'18 mins ago',  adsCount:9,  color:'#3b82f6' },
  { id:3, username:'pk_properties',   site:'OLX Pakistan', status:'error',   lastSync:'3 hrs ago',    adsCount:6,  color:'#ef4444' },
]

export default function Accounts() {
  const [accounts, setAccounts] = useState(INITIAL_ACCOUNTS)
  const [showModal, setShowModal] = useState(false)
  const [form, setForm]       = useState({ username:'', password:'', site:'OLX Pakistan' })
  ...
  const handleConnect = (e) => {
    e.preventDefault()
    const newAcc = { id: Date.now(), username: form.username, site: form.site, status:'active', lastSync:'Just now', adsCount:0, color:'#10b981' }
    setAccounts(prev => [...prev, newAcc])
    setShowModal(false)
    setForm({ username:'', password:'', site:'OLX Pakistan' })
  ...
  return (
    <div style={{ display:'flex' }}>
      ...
      <main className="main-layout">
        <Header title="Connected Accounts" subtitle="Manage your classified ad platform accounts" />
        <div className="page-content fade-in">
          {/* Top bar */}
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:24 }}>
            <div style={{ fontSize:13, color:'var(--text-muted)' }}>{accounts.length} account{accounts.length!==1?'s':''} connected</div>
            ...
            {accounts.map(acc => (
              <div key={acc.id} style={{ background:'var(--bg-card)', border:'1px solid var(--border)', borderRadius:16, padding:24, transition:'all 0.2s', position:'relative', overflow:'hidden' }}
                ...
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12, marginBottom:16 }}>
                  <div style={{ background:'rgba(15,23,42,0.5)', borderRadius:10, padding:'10px 14px' }}>
                ...
                {acc.status==='error' && (
                  <div style={{ marginTop:12, display:'flex', alignItems:'center', gap:6, fontSize:11, color:'#f87171' }}>
                    <AlertCircle size={12} /> Session expired — reconnect to restore
                  </div>
                )}
              </div>
            ))}
          ...
          <div className="modal-box" onClick={e=>e.stopPropagation()}>
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:24 }}>
              <div>
                ...
                <p style={{ fontSize:13, color:'var(--text-muted)', marginTop:4 }}>Your credentials are encrypted and never stored in plain text</p>
              </div>
            ...
            <form onSubmit={handleConnect}>
              ...
              <div style={{ background:'rgba(245,158,11,0.06)', border:'1px solid rgba(245,158,11,0.15)', borderRadius:10, padding:'10px 14px', marginBottom:20, fontSize:12, color:'#fbbf24' }}>
                🔒 Connection uses isolated sessions with proxy rotation (DataImpulse) for safety
              </div>
              <div style={{ display:'flex', gap:10 }}>
                ...
                <button type="submit" className="btn-primary" style={{ flex:1, justifyContent:'center' }}>Connect</button>
              </div>
            </form>
          </div>
        </div>
