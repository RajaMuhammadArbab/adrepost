...
import { Check, Zap, Crown, Building2, Rocket, CreditCard, Clock, Lock } from 'lucide-react'
...
const PLANS = [
  ...
  },
  {
    ...
    features:['Up to 20 ads','3 connected accounts','Repost every 1h','Full logs & analytics','Priority support','Proxy rotation enabled'],
    ads:20, accounts:3
  },
  {
...
const CREDIT_PACKS = [
  { credits:50,  price:'$3.00',  popular:false },
  { credits:150, price:'$7.00',  popular:true  },
  { credits:400, price:'$15.00', popular:false },
]
...
export default function Subscription() {
  ...
  const handleSubscribe = async (plan) => {
    ...
    setLoading(null)
    showToast(`Redirecting to Stripe checkout for ${plan.name} plan... (Demo Mode)`)
  }
  ...
  const handleBuyCredits = async (pack) => {
    ...
    await new Promise(r=>setTimeout(r, 1500))
    setLoading(null)
    showToast(`Redirecting to Stripe checkout for ${pack.credits} credits... (Demo Mode)`)
  }
  ...
  return (
    <div style={{display:'flex'}}>
      ...
      <main className="main-layout">
        <Header title="Subscription & Billing" subtitle="Manage your plan, credits, and Stripe payments" />
                ...
                {PLANS.map(plan=>(
                  <div key={plan.id} className={`plan-card ${plan.featured?'featured':''}`}>
                    ...
                    <button onClick={()=>handleSubscribe(plan)} disabled={loading===plan.id}
                      ...
                      {loading===plan.id ? 'Connecting to Stripe...' : (user?.plan===plan.id ? '✓ Current Plan' : `Subscribe to ${plan.name}`)}
                    </button>
                  </div>
                ))}
              </div>
          ...
          {tab==='credits' && (
            <div>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}>
                <p style={{fontSize:13,color:'var(--text-muted)'}}>Buy credit packs — 1 credit = 1 auto-repost action.</p>
              ...
              <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:16,marginBottom:32}}>
                {CREDIT_PACKS.map(pack=>(
                  <div key={pack.credits} style={{background:'var(--bg-card)',border:`1px solid ${pack.popular?'var(--accent-blue)':'var(--border)'}`,borderRadius:16,padding:24,textAlign:'center',position:'relative',transition:'all 0.2s'}}
                    onMouseEnter={e=>{e.currentTarget.style.transform='translateY(-3px)';e.currentTarget.style.boxShadow='0 12px 30px rgba(0,0,0,0.05)'}}
                    ...
                    <button className="btn-primary" style={{width:'100%',justifyContent:'center'}} onClick={()=>handleBuyCredits(pack)} disabled={loading===pack.credits}>
                      <CreditCard size={15}/> {loading===pack.credits ? 'Loading Stripe...' : 'Buy via Stripe'}
                    </button>
                  </div>
                ))}
              </div>
            ...
            <div style={{background:'var(--bg-card)',border:'1px solid var(--border)',borderRadius:16,overflow:'hidden'}}>
              <table className="data-table">
                ...
                <tbody>
                  {TX_HISTORY.map(tx=>(
                    <tr key={tx.id}>
                      <td style={{color:'var(--text-muted)',fontSize:12}}>inv_strp_{tx.id}X9A</td>
